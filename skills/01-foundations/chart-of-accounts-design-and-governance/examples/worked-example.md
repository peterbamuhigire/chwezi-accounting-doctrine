# Worked Example - Chart Of Accounts Design And Governance
...
An entity asks the finance engine to process a matter involving foundation policy, master-data governance, period state, framework routing, and ledger control. The preparer provides a source document, account map, reporting period, functional currency, and management explanation. The source-register snapshot contains no final statutory value for this example, so no statutory amount is released.
