...
metadata:
  portable: true
  category: 04-subledgers-and-operations
  compatible_with:
    - claude-code
    - codex
---
...
Use when a finance, accounting, reporting, controls, systems, or sector workflow needs cash, POS, imprest, staff-claim, and operational subledger controls that reconcile to the GL.
...
- examples/uganda-worked-example.md shows a Ugandan NGO imprest float run (float, single-transaction cap, replenishment trigger, replenishment cheque, surprise count) using illustrative org-policy figures, not statutory rates.
...
| Reviewer roles | Controller; controls owner; sector specialist where POS or cash operations are affected |
