# Worked Example - Expense Management And Staff Claims
...
An entity asks the finance engine to process a matter involving cash, POS, imprest, staff-claim, and operational subledger controls that reconcile to the GL. The preparer provides a source document, account map, reporting period, functional currency, and management explanation. The source-register snapshot contains no final statutory value for this example, so no statutory amount is released.
